...
p + 
  geom_point() +
  geom_smooth(method="lm", linewidth=1, linetype=2) +
  ...
  labs(title=sprintf("PRELIMINARY: Upfront offers and rate card estimates (%s UberX screenshots)", nrow(tbp)),
...
p + 
  ...
  annotate("label", x=0.1 * max(tbp$estimated_distance),
           y=0.7 * min(tbp$upfront_diff),
           label="Upfront pays less") +
  labs(title=sprintf("PRELIMINARY: Upfront offers and trip distances (%s UberX screenshots)", nrow(tbp)),
...
p + 
  ...
  annotate("label", x=0.1 * max(tbp$estimated_distance),
           ...
           label="Upfront pays less") +
  labs(title=sprintf("PRELIMINARY: Upfront change for short trips (%s UberX screenshots)", nrow(tbp)),
       x="Estimated distance (km)",
       y="Change from rate card to Upfront ($)",
      caption="Estimated rate card fare = 0.75 * ($0.18/min + $0.81/km + $3.17)\nPoints below the grey line show upfront offers lower than rate card would give.") + 
  scale_x_continuous(limits=c(0, x_max)) +
...
p + 
  ...
  labs(title="PRELIMINARY: Upfront offers and rate card estimates",
       subtitle=sprintf("%s UberX screenshots with estimated distance between %s km and %s km",
                        nrow(tbp), distance_min, distance_max),
       x="Estimated rate card fare ($)",
...
p + 
  ...
  annotate("segment", x = 0, xend = x_max, y = 0, yend = x_max,
           colour = "grey", linetype=2) +
  labs(title=sprintf("PELIMINARY: UberXL upfront offers and rate card estimates (%s screenshots)", nrow(tbp)),
       x="Estimated rate card fare ($)",
       y="Upfront fare offer ($)",
...
p + 
  ...
  labs(title="PRELIMINARY: Regular and Trip Radar upfront offers",
