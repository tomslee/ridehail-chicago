...
tbp <- tb %>% 
  ...
  pivot_longer((cols=c("deviation", "mean_miles"), 
                names_to="category", values_to="value")))
...
highlight <- tbp %>% filter(pickup_community_area %in% c(56, 76))
...
p +
  ...
  geom_text(data=highlight, label="Airport", size=4, vjust=2, colour="black") +
  # theme(legend.title=element_blank()) +
  scale_colour_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2")
```
...
tbp <- tb %>% 
  ...
  mutate(this_date=as.Date(sprintf("%s-%s-01", year, month))) %>%
  mutate(area_num_1 = as.character(pickup_community_area),
         map_label=ifelse(deviation > 1.5, area_num_1, ""))
...
p + 
  ...
  labs(title="Average deviation from best fit time-and-distance fare",
       subtitle="Trips categorized by pickup community area",
       fill="Average deviation\n($/trip)") +
