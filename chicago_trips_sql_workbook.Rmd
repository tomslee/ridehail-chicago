...
librarian::shelf(tidyverse, knitr, reshape2, ggtext, devtools,
                 jsonlite, stringr, lubridate, scales, paletteer,
                 ggthemes, RColorBrewer, viridis, vroom,
                 janitor, RPostgres, pals, ggh4x, sf
                 )
...
p <- ggplot(data=tb, aes(x=trip_date, y=trips, 
                             colour="dummy"))
p + 
  #facet_wrap(vars(value), scales="free") +
  geom_line() +
...
```{r ca_aggregate_plot-1, echo=FALSE}
pickup_community_area_list = list(0)
index = 0
...
mean_trip_miles = 6.112
mean_trip_minutes = 18.260
sql <- {"select year, month, avg(fare) as mean_fare,
...
p +
  # facet_wrap(vars(category), scales = "free_y") +
  geom_line() +
  labs(title="Chicago ridehail trends: mean and median fares",
       x="Date",
       y="Value ($)") +
...
# Analysis: model RSE and coefficients
...
sql <- {"select model_date, bin_max, 
  avg(fare) as mean_bin_fare, 
  avg(trip_miles) as mean_bin_miles, 
	avg(60.0 * trip_miles/trip_minutes) as mean_bin_mph, 
	cast(count(*) as numeric) as trip_count, 
	avg(coeff_per_mile * trip_miles + coeff_per_minute * trip_minutes) as mean_bin_model_fare
from
	(select model_date, 
	  trip_miles, trip_minutes, 
	  fare, coeff_per_mile, coeff_per_minute,
	...
	and ridehail.trip_model.model_type = 'lm'
...
# model_diff is the difference between the actual mean fare (for that bin) and the 
# best time-and-distance fare. As the time-and-distance fare comes from the best 
# fit model for the whole distribution, these numbers should average out to zer
# over all bins. A value of model_diff above zero says that that bin has had its
# price raised relative to other bins.
tb_plot <- tb %>%
  mutate(model_diff = mean_bin_fare - mean_bin_model_fare) %>%
  select(!c(mean_bin_miles, trip_count)) %>%
...
## How has the price for trips of different lengths changed?



```{r model_fares_change, echo=FALSE}

sql <- {"select model_date, bin_max, trip_count,
	mean_bin_fare,
	mean_bin_fare/mean_bin_miles as mean_fare_per_mile,
	mean_bin_model_fare,
...
## Does the mean of each fit equally well? (stats questions)


```{r model_fares_distance-fit-mean, echo=FALSE}

sql <- {"select model_date, trip_length_bin, 
  avg(fare) as mean_bin_fare, 
  avg(trip_miles) as mean_bin_miles, 
	avg(60.0 * trip_miles/trip_minutes) as mean_bin_mph, 
	cast(count(*) as numeric) as trip_count, 
	avg(coeff_per_mile * trip_miles + coeff_per_minute * trip_minutes + coeff_intercept) as mean_bin_model_fare
from
	(select model_date, 
	  trip_miles, trip_minutes, 
	  fare, coeff_per_mile, coeff_per_minute, coeff_intercept, 
	  -- round((5 + trip_miles)/ 10) * 10 as bin_max
	  case when trip_miles < 30 then 'short' else 'long' end as trip_length_bin
	from ridehail.trip_fares 
	inner join ridehail.trip_model
	on ridehail.trip_fares.year = date_part('year', ridehail.trip_model.model_date)  
	and ridehail.trip_fares.month = date_part('month', ridehail.trip_model.model_date)  
	...
	-- and ridehail.trip_fares.trip_miles < 40
	and trip_minutes > 0) as T
group by trip_length_bin, model_date
order by trip_length_bin, model_date"}
...
tb_plot <- tb %>%
  ...
  mutate(model_diff =  (mean_bin_fare -  mean_bin_model_fare),
         trip_length_bin = factor(trip_length_bin, levels=c("short", "long"))) %>%
  select(c(model_date, trip_length_bin, model_diff)) %>%
  arrange(model_date, trip_length_bin)

p <- ggplot(data=tb_plot, 
            mapping=aes(x=model_date, 
                        y=model_diff, 
                        fill=trip_length_bin,
                        colour=trip_length_bin))
p +
  geom_col(stat="identity", position="dodge", colour="white") +
  # geom_line() +
  # geom_smooth(method="lm", se=FALSE, fullrange=FALSE, level=0.95) +
  labs(title="Chicago ridehail trends: upfront effects on trips of different distances",
       x="Date",
       y="Actual fare vs time-and-distance fare ($)",
       fill="Trip distance class") +
  # scale_y_continuous(limits=c(0, NA)) +
  # theme(legend.position="none") +
  # theme(legend.title=element_blank()) +
  scale_colour_brewer(palette = "Dark2") +
...
```
## Fare deviations from model (not a useful plot)

```{r fig-win-loss, echo=FALSE}
target_year = 2022
target_month = 6
sql <- {"select dense_rank() over (order by upfront_added) as trip_index,
fare, time_and_distance_fare, upfront_added 	
from
(select f.year, f.month, fare, 
	(coeff_per_mile * trip_miles + coeff_per_minute * trip_minutes) as time_and_distance_fare,
	fare - (coeff_per_mile * trip_miles + coeff_per_minute * trip_minutes) as upfront_added
from ridehail.trip_fares f inner join ridehail.trip_model m
on f.year = m.year and f.month = m.month
where m.pickup_community_area = 0
and m.model_type = 'zero'
and f.year = $1 and f.month = $2 ) as T
order by trip_index;"}

rs <- dbSendQuery(con,sql)
dbBind(rs, list(target_year, target_month))
tb <- as_tibble(dbFetch(rs, n=-1)) 
dbClearResult(rs)

p <- ggplot(data=tb, #%>% mutate(fare_date=as.Date(sprintf("%s-%s-01", year, month))), 
            mapping=aes(x=trip_index, 
                        y=upfront_added,
                        colour="dummy"))

p +
  geom_line() +
  stat_difference(aes(ymin = 0, ymax = upfront_added)) +
  # geom_col(position = "dodge", alpha=0.7) +
  labs(title=sprintf("Chicago fare distributions for %s %s", month.name[target_month], target_year),
       x="Trip index",
       y="Upfront added") +
  scale_y_continuous(limits=c(-20, +20)) + # , trans='log10') +
  theme(legend.position="none") +
  # theme(legend.title=element_blank()) +
  #scale_fill_manual(values=as.vector(pals::alphabet())) +
  #scale_colour_manual(values=as.vector(pals::alphabet()))
...
p + 
  ...
  geom_ribbon(data=subset(mydata, x<=2.5), 
              aes(ymin=7.5, ymax=demand), fill="#d95f02", alpha=0.2,) +
  annotate("segment", x = 0, y = 5, xend = 5, yend = 5, linetype="dashed", color="#1b9e77") +
  annotate("segment", x = 5, y = 0, xend = 5, yend = 5, linetype="dashed", color="#1b9e77")  +
  annotate("segment", x = 0, y = 7.5, xend = 2.5, yend = 7.5, linetype="dashed", color="#d95f02") +
  annotate("segment", x = 2.5, y = 2.5, xend = 2.5, yend = 7.5, linetype="dashed", color="#d95f02")  +
  annotate("label", x = 9, y = 1, label = "Demand") + #, label.size=0) +
  annotate("label", x = 9, y = 9, label = "Supply") +
  annotate("label", x = 5, y = 5, hjust="left", label = "Competition", colour="#1b9e77") +
  annotate("label", x = 1.25, y = 6, hjust="left", label = "Consumer surplus\n(competition)", fill="white", colour="#1b9e77") +
  annotate("label", x = 2.5, y = 7.5, hjust="left", label = "Monopoly", colour="#d95f02") +
  annotate("label", x = 0.25, y = 8.5, hjust="left", label = "Consumer surplus\n(monopoly)", colour="#d95f02") +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
...
p +
  ...
  scale_colour_brewer(palette = "Dark2")
```
# Community Area-based deviations

```{r ca_deviation, echo=FALSE}
...
tbp <- tb %>% 
  ...
  pivot_longer(cols=c( "mean_miles", "deviation"), 
                names_to="category", values_to="value")

p <- ggplot(data=tbp, 
            mapping=aes(x=pickup_community_area, 
                        y=value, 
                        colour=category,
                        fill=category))
...
##  Community Area Scatter Plot

```{r deviation-more, echo=FALSE}
# continuation
...
highlight <- tbp %>% filter(deviation > 1.5)

p +
  geom_point() +
  geom_hline(yintercept=0, colour="darkgrey") +
  theme(legend.position="none") +
  labs(title="Algorithmic Pricing: Pickup Community Area",
       subtitle="Dot size shows the number of trips from the community area",
       x="Average trip distance (miles)",
       y="Deviation from time-and-distance model ($/trip)") +
...
## O'Hare and others over time

```{r label=ohare, echo=FALSE}

tbp <- tb %>% 
  filter(pickup_community_area %in% c(8, 32, 33, 76)) %>% 
  arrange(year, month) %>%
  mutate(this_date=as.Date(sprintf("%s-%s-01", year, month)))

p <- ggplot(data=tbp, 
            mapping=aes(x=this_date, 
                        y=deviation, 
...
p + 
  ...
  geom_line() +
  labs(title="Deviation from time-and-distance fares, by community area",
       subtitle="Shows selected high-price community areas",
       x="Date",
       y="Deviation ($/trip)",
       colour="Community area",
       fill ="Community area") +
  scale_x_date() +
  scale_colour_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2")
...
tbp <- tb %>% 
  ...
  mutate(map_label=ifelse(deviation > 1.5, area_num_1, "")) 
...
p + 
  ...
  labs(title="Average deviation from best fit time & distance fare",
       subtitle="Trips categorized by pickup community area",
       fill="Deviation\n($/trip)") +
  # scale_fill_continuous(low = "ivory", high = "#d95f02")
  scale_fill_continuous(low = "ivory", high = "#1b9e77") +
  theme_map() # +
  # theme(legend.position="right")
```

# Hour-of-day dependence (not much here)

Not getting much from here, to be honest.
...
select F.year, F.month,
  -- F.pickup_community_area, 
  -- CA.name as ca_name,
  date_part('hour', trip_start_timestamp::timestamp) as hour_of_day,
  cast(count(*) as decimal) trips, 
  avg(fare) as mean_fare, 
  avg(trip_miles) as mean_miles, 
  avg(trip_minutes) as mean_minutes, 
  avg(fare - (coeff_per_mile * trip_miles + coeff_per_minute * trip_minutes + coeff_intercept)) as deviation
...
inner join ridehail.trip_model M
  ...
  and F.month = $2
group by F.year, F.month, -- F.pickup_community_area, CA.name, 
date_part('hour', trip_start_timestamp::timestamp)
-- having count(*) > 100
...
this_community_area = 0

tbp <- tb %>% 
  mutate(deviation_scale = deviation * trips) %>%
  # filter(pickup_community_area == this_community_area) %>%
  pivot_longer(cols=c("trips",  "deviation", "deviation_scale", "mean_fare", "mean_miles", "mean_minutes"), 
                names_to="category", values_to="value")

p <- ggplot(data=tbp, 
            mapping=aes(x=hour_of_day, 
                        y=value, 
                        colour=category,
                        fill=category))
...
p +
  facet_wrap(vars(category), scales="free") +
  geom_col(position="identity", alpha=0.3) +
  # labs(title=sprintf("Time of day dependence for CA %s (%s)", this_community_area, tbp$ca_name)) +
  theme(legend.position="none") +
  # theme(legend.title=element_blank()) +
  scale_colour_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2")
