...
"rate card" calculation? (Note: "upfront fares" is different to "upfront rates" that now control how 
much drivers earn on a trip in many US markets).
...
The data collected is for the first Thursday of February, June, and October for each year. I collected it by downloading a query result from the Chicago web site and uploading it to PostgreSQL using the pgsql \\copy command. It is stored in a table "ridehail.trip_fare" and models are built from a cleaned data set accessed through the view "ridehail.trip_fares", which filters out trips with missing fields that are important for the models.

```{r model, echo=FALSE}
pickup_community_area_list = list(32)
years <- 2019:2024
months <- list(2, 6, 10)
  ...
  for (this_year in years) {
    for (this_month in months){
        ...
        if(nrow(tb_sql) > 0){
          ...
          tb_result <- tb_agg %>% 
            mutate(coeff_per_mile=miles,
                   coeff_per_minute=minutes,
...
tb <- as_tibble(dbFetch(rs, n=-1))
tb <- tb %>% slice_min(trip_percent, n=10)
dbClearResult(rs)
...
Take a busy downtown community area (8, 32) and compare it to a low-traffic area (47, 55).

TODO: Better to compare to the total for each month, and probably a more concise query.
...
# Analysis: aggregate and modelled information
