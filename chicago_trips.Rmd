...
```
# Percent change in trips, year over year

```{r plot-yoy-trips, echo=FALSE}
tb <- trips %>%
  arrange(year, month) %>%
  filter(category=="trip_count") %>%
  mutate(last_year = lag(measure, n=12),
         yoy=100* (measure - last_year)/last_year) %>%
  filter(year >= 2022)

p <- ggplot(data=tb,
            mapping=aes(x=date, y=yoy, colour=category, fill=category))
...
p +
  ...
  theme(legend.position="none") +
  scale_y_continuous(limits=c(0, NA), labels=label_number(scale_cut=cut_short_scale())) +
  labs(title="Uber Chicago Trip Growth (%, YoY)", 
       subtitle="Each column represents the year-over-year growth in trips on the first Thursday of the month",
       x="Date",
       y="Year-over-year growth in number of trips (%)") +
  scale_colour_brewer(palette="Dark2") +
