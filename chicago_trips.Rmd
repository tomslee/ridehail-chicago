...
TNC_TRIPS_ENDPOINT="https://data.cityofchicago.org/resource/m6dm-c72p.json"
TNC_TRIPS_ENDPOINT_2023="https://data.cityofchicago.org/resource/n26f-ihde.json"
...
res <- GET(TNC_TRIPS_ENDPOINT_2023, add_headers(app_token=APP_TOKEN))
...
w <- URLencode("trip_start_timestamp between '2023-01-01T00:00:00.000' and '2023-03-01T00:00:00.000'")
url <- sprintf("%s?$where=%s&$order=trip_start_timestamp", TNC_TRIPS_ENDPOINT_2023, w)
...
#  for (month in c(1:12)){
 for (year in c(2023)) {
  for (month in c(01, 02)){
    ...
    url <- URLencode(sprintf("%s?$query=%s", TNC_TRIPS_ENDPOINT_2023, q))
    res <- GET(url, add_headers(app_token=APP_TOKEN))
...
trips <- read_csv("./data/trip_aggregates.csv", col_types="iiidddd") %>%
  ...
  pivot_longer(c(trip_count, fare_per_minute, trip_total_per_minute,
                 fare_per_mile, miles_per_hour, minutes, miles),
               names_to="category", values_to="measure"
               )

tb <- trips %>%
  filter(date > as.Date("2018-12-30"))
...
p +
  ...
  scale_colour_discrete(labels=c("Gas price ($/gal)", "Trip total fare ($/min)")) +
  theme (legend.position=c(0.2, 0.8), 
        legend.title=element_blank()) +
  scale_colour_tableau(palette="Tableau 10")
```

# Hourly trends during the day
...
p +
  ...
  labs(title=sprintf("Chicago ridehail: trip distribution on the first Thursday in February", test_date),
