...
p + 
  ...
  labs(title="Full-time ridehail drivers in Chicago",
       subtitle=sprintf("Full timers (over %s trips per month) are a small fraction of drivers, but drive about half of all trips.\nThe importance of full-time drivers has been slowly growing over time.", full_time_trips),
       x="Date",
       y="Percent",
       colour="Driver category") +
  scale_y_continuous(limits=c(0, 60), breaks=waiver(), n.breaks=6) +
...
reference_month = as.Date("2023-05-01")
...
tb <- active_drivers %>% 
  filter(
    ...
    driver_start_month <= as.Date("2022-06-01"),
